...
// 	ang = atan(cy->radius / cy->height);
// 	x = vector_subtract(r.orig,
// 		ray_at(create_ray(cy->pos, cy->norm), -cy->height / 2));
// 	a = pow(cos(ang), 2) * vector_square_length(vector_subtract(r.dir,
// 		vector_multiply(cy->norm, vector_dot(r.dir, cy->norm)))) -
// 		pow(sin(ang), 2) * pow(vector_dot(r.dir, cy->norm), 2);
// 	b = 2 * (pow(cos(ang), 2) * vector_dot(vector_subtract(r.dir,
// 		vector_multiply(cy->norm, vector_dot(r.dir, cy->norm))),
// 		vector_subtract(x, vector_multiply(cy->norm, vector_dot(x, cy->norm)))) -
// 		pow(sin(ang), 2) * vector_dot(r.dir, cy->norm) * vector_dot(x, cy->norm));
// 	c = pow(cos(ang), 2) * vector_square_length(vector_subtract(x,
// 		vector_multiply(cy->norm, vector_dot(x, cy->norm)))) -
// 		pow(sin(ang), 2) * pow(vector_dot(x, cy->norm), 2);
