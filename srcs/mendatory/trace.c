...
static t_color	specular_reflection(t_main *data, t_hit hit, t_point l_dir)
{
	t_ray	v_ray;
	t_point	h_dir;
	t_color	light;
	...
	v_ray = create_ray(hit.hit_pos, vector_subtract(data->camera.pos, hit.hit_pos));
	h_dir = unit_vector(vector_add(v_ray.dir, l_dir));
	nh  = vector_dot(hit.hit_norm, h_dir); 
	if (nh < 0)
		nh = 0;
	light = color_multiply_n(data->light.color,
		data->light.ratio * pow(nh, SHININESS));
	return (light);
}

// until page-17 https://www.cs.cornell.edu/courses/cs4620/2014fa/lectures/05rt-shading.pdf
static t_color	get_shaded_color(t_main *data, t_hit hit)
{
	...
	if (!hit_object(data, l_ray).is_hit) // not blocked
	{
		light = color_add(light, diffuse_reflection(data, hit, l_dir));
		light = color_add(light, specular_reflection(data, hit, l_dir));
	}
