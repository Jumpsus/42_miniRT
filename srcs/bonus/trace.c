...
static t_color	specular_reflection(t_hit hit, t_object camera, t_object *l_obj, t_point l_dir)
{
	t_ray	v_ray;
	t_point	h_dir;
	t_color	light;
...
static t_color	get_shaded_color(t_main *data, t_hit hit)
{
	...
	while (l_obj)
	{
		...
		if (!hit_object(data, l_ray).is_hit) // not blocked
		{
			...
			light = color_add(light, specular_reflection(hit, data->camera, l_obj, l_dir));
		}
		l_obj = l_obj->next;
	}
